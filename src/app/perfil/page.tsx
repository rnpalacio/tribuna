  ...
  return (
    <main className="app-shell bg-cream text-ink min-h-screen">
      <div className="px-5 pt-12 pb-28">
        ...
        {authed && (
          <>
            <h2 className="text-xs tracking-[0.15em] text-black/45 font-semibold mb-3">TUS PUNTOS</h2>
...
function PointBox({ label, value, highlight = false }: { label: string; value: number; highlight?: boolean }) {
  return (
    <div className={`rounded-2xl p-4 text-center ${highlight ? "bg-ink text-white" : "bg-white"}`}>
      <p className={`display text-3xl ${highlight ? "" : "text-brand"}`}>{value.toLocaleString("es-CL")}</p>
      <p className={`text-[11px] mt-1 ${highlight ? "text-white/60" : "text-black/45"}`}>{label}</p>
    </div>
  );
}
