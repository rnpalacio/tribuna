...
export function PollCard({ poll, dark = true }: { poll: Poll; dark?: boolean }) {
  ...
  const showResults = !!voted || revealed;
          ...
          return (
            ...
            >
              ...
              <span className="relative flex justify-between items-center font-semibold">
                <span>{o.label}</span>
                {showResults && <span className="text-sm opacity-80">{pct}%</span>}
              </span>
            </button>
          );
        })}
      ...
      <div className="flex items-center justify-between mt-3 gap-3">
        ...
        {!showResults && (
          <button
            ...
            className={`text-xs font-semibold shrink-0 ${dark ? "text-brand" : "text-brand"}`}
          >
            Ver resultados
          </button>
        )}
